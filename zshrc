...
# Source secrets (API tokens, etc.) — not synced via dotfiles
...
alias push-zsh='cd ~/dotfiles && cp ~/.zshrc zshrc && git add zshrc && git commit -m "Update .zshrc - $(date +%Y-%m-%d\ %H:%M)" && git push origin main && cd - > /dev/null'
